...
pub fn setup_tray(app: &AppHandle) -> tauri::Result<()> {
    ...
    let items: &[&dyn IsMenuItem<tauri::Wry>] = &[
        &open, &settings, &sep, &start, &stop, &restart, &sep, &browser, &sep, &quit,
    ];
    ...
    *app.state::<AppState>().tray_state.lock().unwrap() = Some(TrayState {
    ...
    let icon =
        tauri::image::Image::from_bytes(include_bytes!("../resources/tray/tray-icon.png")).ok();
...
pub fn show_main_window(app: &AppHandle) -> Result<(), String> {
    ...
    if running {
        crate::window::ensure_window(
            app,
            &crate::commands::harness_web_url(
                app,
                current_port(app),
...
mod language_tests {
    ...
    fn every_language_labels_every_item() {
        for lang in ["en", "zh-Hant", "zh-Hans", "ja", "es"] {
            let l = labels_for(lang);
            for label in [
                l.open, l.settings, l.start, l.stop, l.restart, l.browser, l.quit,
            ] {
