...
(function () {
  ...
  var FIND_BAR_ID = 'dsh-fz-bar';
  var ZOOM_BADGE_ID = 'dsh-fz-zoom';
  var ZOOM_KEY = 'dsh-zoom';
  var MIN_ZOOM = 50;
  var MAX_ZOOM = 200;
  var ZOOM_STEP = 10;
  ...
  function readZoom() {
    ...
    } catch (e) { /* storage unavailable: session-only zoom */ }
    if (isNaN(z)) z = 100;
  ...
  function applyZoom(z) {
    ...
    } catch (e) { /* headless/shadow contexts */ }
    try {
  ...
  function showZoomBadge(z) {
    var badge = document.getElementById(ZOOM_BADGE_ID);
    if (!badge) return;
    badge.textContent = z + '%';
  ...
  // ---- Find bar ------------------------------------------------------------
  var lastQuery = '';
  var searchedQuery = null; // query currently reflected by selection/highlights
  var lastIndex = 0; // 1-based index of the current match for the count label
  var totalMatches = 0;
  ...
  function countMatches(query) {
    ...
    var walker;
    try {
      walker = document.createTreeWalker(document.body || document.documentElement,
    ...
    while (walker.nextNode()) {
      var text = walker.currentNode.nodeValue.toLowerCase();
      var from = 0;
      var at;
  ...
  function highlightAll(query) {
    try {
      clearHighlight();
      if (!query || !('highlights' in CSS) || typeof Highlight === 'undefined' ||
      ...
      while (walker.nextNode() && ranges.length < 1000) {
        ...
        var from = 0;
        var at;
        while ((at = text.indexOf(q, from)) !== -1 && ranges.length < 1000) {
          var r = new Range();
          r.setStart(node, at);
    ...
    } catch (e) { /* highlight is decorative; navigation still works */ }
  }
  ...
  function updateCount() {
    var bar = uiRoot();
    if (!bar) return;
    var label = bar.querySelector('#dsh-fz-count');
    if (!label) return;
    if (!lastQuery) {
      label.textContent = '';
      return;
  ...
  function findNavigate(backwards) {
    ...
    if (nativeFind(lastQuery, backwards)) {
      ...
      if (totalMatches > 0) {
        lastIndex = backwards
          ? (lastIndex <= 1 ? totalMatches : lastIndex - 1)
          : (lastIndex >= totalMatches ? 1 : lastIndex + 1);
      }
    }
    updateCount();
  }
  ...
  function buildBar() {
    ...
    bar.querySelector('#dsh-fz-prev').addEventListener('click', function () { findNavigate(true); });
    bar.querySelector('#dsh-fz-next').addEventListener('click', function () { findNavigate(false); });
  ...
  function injectStyles() {
    ...
    style.textContent =
      '#' + FIND_BAR_ID + ' { all: initial; display: none; position: fixed; top: 12px; right: 12px;' +
      ...
      '#' + FIND_BAR_ID + '.open { display: flex; }' +
      '#' + FIND_BAR_ID + ' * { box-sizing: border-box; }' +
      ...
      ' white-space: nowrap; }' +
      '#' + FIND_BAR_ID + ' button { background: transparent; border: 1px solid transparent;' +
      ' color: #cbd5e1; border-radius: 6px; padding: 4px 8px; font-size: 12px; cursor: pointer; }' +
      '#' + FIND_BAR_ID + ' button:hover { background: #1e293b; }' +
      '#' + FIND_BAR_ID + ' button:focus-visible { outline: 2px solid #60a5fa; outline-offset: 1px; }' +
      '#' + ZOOM_BADGE_ID + ' { all: initial; display: none; position: fixed; right: 16px; bottom: 16px;' +
      ...
      '#' + ZOOM_BADGE_ID + '.on { display: block; }' +
  ...
  window.addEventListener('keydown', function (e) {
    ...
    var inFindInput = e.target && e.target.id === 'dsh-fz-input';
    var key = e.key;
    var code = e.code || '';
  ...
  window.addEventListener('keydown', function (e) {
    ...
    var bar = uiRoot();
    if (bar && bar.classList.contains('open')) closeFind();
  });
})();
