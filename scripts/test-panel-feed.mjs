...
function makeElement(doc, tag) {
  const el = {
    ...
    addEventListener(type, fn) {
      ...
      if (!el._handlers[type]) el._handlers[type] = [];
...
function boot(opts) {
  ...
  Object.defineProperty(g, "location", {
    value: { href: "dsh-ui://localhost/settings" },
    configurable: true,
  });
  ...
  const restore = () => {
    ...
    g.fetch = saved.fetch;
    g.setTimeout = savedTimers.setTimeout;
    g.setInterval = savedTimers.setInterval;
    g.clearTimeout = savedTimers.clearTimeout;
    g.clearInterval = savedTimers.clearInterval;
  };
  return {
    sched,
    doc,
...
function stuckCount(out) {
  return out.children.filter((c) => /no progress for 60s/.test(String(c.textContent))).length;
}
...
const STATUS = {
  ...
  port: 3081,
  host: "127.0.0.1",
  installedVersions: ["0.1.2-rc.1"],
  activeVersion: "0.1.2-rc.1",
};
...
console.log("1. panel script boots and primes status + logs");
{
  ...
  await settle();
  check(
    "get_status invoked at init",
    h.invoked.some((p) => p[0] === "get_status"),
  );
  check(
    "tail_logs invoked at init",
    h.invoked.some((p) => p[0] === "tail_logs"),
  );
  h.restore();
}
...
console.log("2. single-key update lifecycle: feed closes, no stuck line after done");
{
  ...
  h.emit("launcher://progress", {
    op: "update",
    version: null,
    phase: "registry",
    message: "Checking npm registry for latest version…",
    percent: 5,
  });
  ...
  h.emit("launcher://progress", {
    ...
    version: "0.1.2-rc.1",
    phase: "stopping",
    message: "Stopping the running engine to switch versions…",
    percent: 10,
  });
  h.emit("launcher://progress", {
    op: "update",
    version: "0.1.2-rc.1",
    phase: "installing",
    message: "Installing DeepSeek Harness version@0.1.2-rc.1",
    percent: 40,
  });
  ...
  h.emit("launcher://progress", {
    ...
    version: "0.1.2-rc.1",
    phase: "done",
    message: "switched to v0.1.2-rc.1 — engine running on port 3081",
    percent: 100,
  });
  ...
  check(
    "done line recorded",
    out.children.some((c) => /engine running on port 3081/.test(String(c.textContent))),
  );
...
console.log("3. done phase clears a pending stuck timer (reported bug)");
{
  ...
  h.emit("launcher://progress", {
    ...
    version: "0.1.2-rc.1",
    phase: "done",
    message: "switched to v0.1.2-rc.1",
    percent: 100,
  });
...
console.log("5. install dropdown defaults to the newest published version");
{
  ...
  check("no init-time crash", h.error === null, h.error && String(h.error));
  check(
    "dropdown defaults to newest version, not the active one",
    installSel.value === "0.1.2-rc.1",
    `got ${installSel.value}`,
  );
  const opts = installSel.children;
  const activeOpt = opts.find((o) => o.value === "0.1.1-rc.1");
  const latestOpt = opts.find((o) => o.value === "0.1.2-rc.1");
  check("installed option carries the checkmark", /✓/.test(String(activeOpt?.textContent)));
  check("uninstalled latest option has no checkmark", !/✓/.test(String(latestOpt?.textContent)));
  h.restore();
}
...
{
  check(
    "header column renamed to Directory",
    /<span class="vc-dir" title="Open the version directory" data-i18n="directory" data-i18n-title="directoryTitle">Directory<\/span>/.test(
      html,
    ),
  );
  ...
  check(
    "install dropdown + button are the table's last row",
    vtable !== -1 &&
      vtable < installSel &&
      installSel < btnInstall &&
      btnInstall < opFeeds &&
      opFeeds < html.indexOf("</main>"),
  );
  check(
  ...
  check(
    "install row keeps the 4-column layout (empty | dropdown | empty | button)",
    /vrow vinstall-row">\s*<span class="vc-def"[^>]*><\/span>\s*<span class="vc-ver vc-ver-install">\s*<select id="sel-install-ver"/.test(
      html,
    ),
  );
...
console.log("7. stopped download: feed dismisses once; late lines never resurrect it");
{
  ...
  const feedsBox = h.doc.getElementById("op-feeds");
  h.emit("launcher://progress", {
    op: "download:v0.1.2-alpha.5",
    version: "0.1.2-alpha.5",
    phase: "registry",
    message: "Checking DeepSeek Harness versions",
    percent: 5,
  });
  h.emit("launcher://progress", {
    op: "download:v0.1.2-alpha.5",
    version: "0.1.2-alpha.5",
    phase: "installing",
    message: "Installing DeepSeek Harness version@0.1.2-alpha.5",
    percent: 40,
  });
  const feedEl = feedsBox.children[feedsBox.children.length - 1];
  check(
    "download feed shows its command header",
    feedEl.hidden === false && feedEl._qs.get(".op-feed-head").hidden === false,
  );
  ...
  check("feed removed after the first cancelled window (no extension)", feedEl.parentNode === null);
  ...
  h.emit("launcher://console", {
    op: "download:v0.1.2-alpha.5",
    stream: "out",
    text: "npm warn cancel cleanup line",
  });
  h.emit("launcher://console", { op: "download:v0.1.2-alpha.5", stream: "err", text: "npm error canceled" });
  check("late console lines for a closed op do not re-create the feed", feedsBox.children.length === 0);
  h.sched.advance(61000);
  check(
    "no stuck timer fired for the closed op",
    stuckCount({ children: [] }) === 0 && feedsBox.children.length === 0,
  );
  await settle();
  h.restore();
}
...
console.log("9. minimal terminal: header for download phases only, no stop control");
{
  ...
  const feedEl = feedsBox.children[0];
  check(
    "registry feed visible without header",
    feedEl.hidden === false && feedEl._qs.get(".op-feed-head").hidden === true,
  );
  check(
    "registry message streamed as info line",
    feedEl._qs.get(".term-out").children.some((c) => /Checking npm registry/.test(String(c.textContent))),
  );
  ...
  check(
    "terminal header is action-neutral",
  ...
  check("notice feed has no header", noticeEl._qs.get(".op-feed-head").hidden === true);
  h.sched.advance(10000);
  await settle();
  h.restore();
}
...
{
  check(
    "no stop button in the panel markup",
    !html.includes("dl-cancel") && !/aria-label="Stop download"/.test(html),
  );
  check("no stop glyph reference remains", !/data-ico="stop-(fill|circle)"/.test(html));
  const fs = await import("node:fs");
  const path = await import("node:path");
  const url = await import("node:url");
  const root = path.resolve(path.dirname(url.fileURLToPath(import.meta.url)), "..");
  check(
    "stop-fill.svg removed from resources",
...
console.log("10. tray has no fullscreen items; green button owns fullscreen");
{
  ...
  for (const id of ["fullscreen-harness", "fullscreen-settings"]) {
    check(
      `no tray symbol PNG for ${id}`,
      !assetExists(path.join(root, "src-tauri", "resources", "tray", "symbol", `${id}.png`)),
    );
    check(`no tray menu item for ${id}`, !tray.includes(`"${id}"`));
  }
  check("no fullscreen toggle handler remains", !/toggle_fullscreen/.test(tray));
  check(
    "no fullscreen label entries remain",
    !/fullscreen_harness|fullscreen_settings|Full Screen/.test(tray),
  );
  const symbols = readFileSync(path.join(root, "scripts", "gen-symbols.mjs"), "utf8");
  check("tray browser item maps to the SF link symbol", /\["browser",\s*"link"\]/.test(symbols));
  check("arrow.up.right.square retired from the tray", !symbols.includes("arrow.up.right.square"));
  const win = readFileSync(path.join(root, "src-tauri", "src", "window.rs"), "utf8");
  check("green-button fullscreen flag still applied at window creation", /FullScreenPrimary/.test(win));
}
...
console.log("9. versions table renders hostile version names as inert text");
{
    ...
    invokeImpl: (cmd) => {
      if (cmd === "get_status")
        return {
          ...
          host: "127.0.0.1",
          versions: [],
          installedVersions: ["0.1.1-rc.1", EVIL],
          activeVersion: "0.1.1-rc.1",
          latestRemote: null,
        };
  ...
  check(
    "hostile version name present as text",
...
{
  check(
    "refresh button in install row markup with refresh glyph",
    html.includes('id="btn-refresh-versions"') && html.includes('data-ico="arrow-clockwise"'),
  );
  let glyphOk = false;
  try {
    readFileSync(path.join(root, "src-tauri", "resources", "bootstrap-icons", "arrow-clockwise.svg"));
    glyphOk = true;
  } catch {}
  ...
  const h = boot({
    invokeImpl: (cmd) => {
      if (cmd === "refresh_versions") {
        refreshCalls++;
        extraVersion = true;
        return "version list refreshed (3 versions)";
      }
      if (cmd === "get_status") {
        return {
          enginePhase: "stopped",
  ...
  check("refresh button wired", !!rbtn._handlers?.click?.length);
  rsel.value = "0.1.1";
...
console.log("12. operation terminals head with Harness Launcher #");
{
  ...
  check(
    "overlay terminal title is neutral",
    overlaySrc.includes(">Harness Launcher #</span>") &&
      !overlaySrc.includes("Installing DeepSeek Harness version@"),
  );
}
...
console.log("13. header language switcher persists and repaints");
{
  ...
  check(
    "five language segments with the specified labels",
    segCodes.every((c) => html.includes(`data-lang="${c}"`)) && segLabels.every((s) => html.includes(s)),
  );
  ...
  check(
    "status payload drives the panel language",
    /if \(s\.language\) applyLanguage\(s\.language\)/.test(scriptSrc),
  );
  ...
  seg._handlers.click[0]({ target: { closest: () => ({ getAttribute: () => "es" }) } });
  await settle();
  const persisted = hj.invoked.filter((p) => p[0] === "set_language");
  check(
    "set_language invoked once with the picked code",
  ...
  await settle();
  check("unknown saved language resets document lang to en", hx.doc.documentElement.lang === "en");
  hx.restore();
}
